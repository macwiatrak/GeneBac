...
def collect_preds(model: DeepBacGeneExpr, dataloader: DataLoader):
    out = defaultdict(list)
    with torch.no_grad():
        for idx, batch in enumerate(tqdm(dataloader, mininterval=5)):
            logits, gene_embeddings, gene_embeddings_w_pos_enc = model(
            ...
            out["embedding_w_pos_enc"] += [
                item.cpu().numpy() for item in gene_embeddings_w_pos_enc
            ]
...
def run(
    ...
    del test_df  # delete to free up memory
