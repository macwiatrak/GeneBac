...
from deep_bac.data_preprocessing.utils import read_ref_genome, read_genome_assembly, get_gene_data_dict
from deep_bac.data_preprocessing.variants_to_strains_genomes import get_and_filter_variants_df, apply_variants_to_a_gene
...
def agg_variants(variants_df: pd.DataFrame) -> pd.DataFrame:
    agg_df = variants_df.sort_values(by=['NUCLEOTIDE_NUMBER']).groupby(["UNIQUEID", "GENE"]).agg(list)
...
def postprocess_agg_variants(agg_variants_df: pd.DataFrame) -> pd.DataFrame:
    ...
    agg_variants_df = agg_variants_df.drop(columns=['REF', 'ALT', 'NUCLEOTIDE_NUMBER', 'IS_INDEL'])
    # clean up the columns
    agg_variants_df['prom_gene_seq_w_variants'] = agg_variants_df['features'].progress_apply(
        lambda x: x['prom_gene_seq_w_variants'])
    agg_variants_df['ref_correct_ratio'] = agg_variants_df['features'].progress_apply(
        lambda x: x['ref_correct_ratio'])
...
def run(
    ...
    genome_assembly = read_genome_assembly(os.path.join(input_dir, GENOME_ASSEMBLY_FILE_NAME))
    ...
    strain_w_phenotype_ids = get_strain_w_phenotype_ids(os.path.join(input_dir, PHENOTYPE_FILE_NAME))
    ...
    logging.info(f"Mean ref correct ratio: {mean_ref_correct_ratio}"
                 f"Std ref correct ratio: {std_ref_correct_ratio}")

    # save agg variants
    logging.info("Saving agg variants")
    # agg_variants_df.to_parquet(
    #     path=os.path.join(input_dir, output_dir),
    #     engine='pyarrow',
    #     partition_cols=['UNIQUEID'],
    # )
