...
from genebac.data_preprocessing.data_types import BacInputSample
from genebac.data_preprocessing.utils import (
    shift_seq,
    seq_to_one_hot,
    pad_one_hot_seq,
)
...
def transform_dna_seq(
    ...
    one_hot_seq = seq_to_one_hot(seq[:max_gene_length])
    # stochastically do a reverse complement of the sequence
    if random.random() < reverse_complement_prob:
    ...
    random_shift = random.randint(-shift_max, shift_max)  # get random shift
    one_hot_seq = shift_seq(one_hot_seq, random_shift, pad_value)  # shift seq
    # pad one hot seq
    padded_one_hot_seq = pad_one_hot_seq(
        one_hot_seq, max_gene_length, pad_value
    )
    # transpose the one hot seq to make it channels first
    return padded_one_hot_seq.T
...
class BacGenomeGenePhenoDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for idx, gene in enumerate(self.gene_to_id.keys()):
            ...
            padded_one_hot_seq = self.transform_fn(seq)
